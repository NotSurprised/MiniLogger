...
const WCHAR* BackupFolder = L"\\Devise\\HarddiskVolume2\\Backup4Ransomware\\";
const WCHAR* Spot = L":";

const WCHAR* extentions[] = { L".txt", L".doc", L".docx", L".xls", L".zip", L".rar", L".pdf", L".ppt", L".pptx"
, L".mdb", L".mp3", L".mp4", L".mpg", L".png", L".gif", L".bmp", L".jpg", L".jpeg"
, L".ico", L".bmp", L".csv", L".flv", L".wmv" };

int numberOfExtentions = sizeof(extentions) / sizeof(WCHAR*);
...
--*/
{
	...
	BOOLEAN nameSet = FALSE; //check if campare have been set by  FltGetFileNameInformation
...
#pragma prefast(suppress:__WARNING_BANNED_API_USAGE, "reviewed and safe usage")
	...
	BOOLEAN isDir;
	FltIsDirectory(Data->Iopb->TargetFileObject, Data->Iopb->TargetInstance, &isDir);	//&isDir T when directory/not exist, F when file 
	if (nameSet == TRUE && isDir == FALSE)
	{
		for (int i = 0; i < numberOfExtentions; i++)									//check if FileType we Focus
		{
	...
	else if (nameSet == TRUE && isBlock == TRUE && isDir == FALSE)	//campareName already get target IRP path
	{
		if ((Data->Iopb->MajorFunction == IRP_MJ_CREATE))
		{
			if (FltObjects->FileObject->SharedRead == FALSE)		//check if an existing file is being opened for write without sharing read.
			{

			}
			if (FltObjects->Size == 0)								//blank new MS OFFICE temp files
			{

			}
			if (Data->IoStatus.Information == FILE_SUPERSEDE)//file opened with FILE_SUPERSEDE/FILE_OVERWRITE/FILE_OVERWRITE_IF flags
			{

			}
			if (Data->IoStatus.Information == FILE_OVERWRITE)//file opened with FILE_SUPERSEDE/FILE_OVERWRITE/FILE_OVERWRITE_IF flags
			{

			}
			if (Data->IoStatus.Information == FILE_OVERWRITE_IF)//file opened with FILE_SUPERSEDE/FILE_OVERWRITE/FILE_OVERWRITE_IF flags
			{

			}
		}
		if ((Data->Iopb->MajorFunction == IRP_MJ_WRITE) || (Data->Iopb->MajorFunction == IRP_MJ_SET_INFORMATION))	//make back up before modify for file obj that can read.
		{
