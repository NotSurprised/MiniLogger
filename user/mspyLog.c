...
char* IrpMajorFunction;
char* IrpMinorFunction;
...
--*/
{
    ...
    if (OutputFile) {
        ...
        if (irpMinorString) {
			...
			IrpMajorFunction = irpMajorString;
			IrpMinorFunction = irpMinorString;
...
--*/
{
	...
	//
	char SeqNum[11];
	sprintf(SeqNum, "0x%08X", SequenceNumber);
	...
	if (FormatSystemTime(&systemTime, time, TIME_BUFFER_LENGTH)) {
		sprintf(TIME, "%-12s", time);
		//fprintf(File, "\t%-12s", time);

	}
	else {
		sprintf(TIME, "%-12s", TIME_ERROR);
		//fprintf(File, "\t%-12s", TIME_ERROR);
	}
	//printf("time : %-12s\n", time);
	char PId[10];
	sprintf(PId, "%d", RecordData->ProcessId);
	//printf("PId : %s\n", PId);
	//fprintf(File, "\t%8Ix.%-4Ix ", RecordData->ProcessId, RecordData->ThreadId);

	/*HANDLE hProcess = OpenProcess(PROCESS_QUERY_INFORMATION |
		PROCESS_VM_READ,
		FALSE, RecordData->ProcessId);
	TCHAR TProcessName[MAX_PATH] = TEXT("<unknown>");
	if (NULL != hProcess)
		GetProcessImageFileName(hProcess, TProcessName, MAX_PATH);
	char ProcessName[MAX_PATH];
	sprintf(ProcessName, "%S", TProcessName);*/
	//printf("ProcessName : %s\n", ProcessName);
	char *ProcessName = RecordData->ProcessName;
	//printf("ProcessName : %s\n", ProcessName);
	...
	PrintIrpCode(RecordData->CallbackMajorId,
		...
		File,
		TRUE);
	char* MajorFunction = IrpMajorFunction;
	//printf("major : %s\n", MajorFunction);
	char* MinorFunction = IrpMinorFunction;
	//printf("minor : %s\n", MinorFunction);
	...
	char Status[11];
	sprintf(Status, "0x%08lx", RecordData->Status);
	//printf("%s \n", Status);

	char Information[19];
	sprintf(Information, "0x%p", (PVOID)RecordData->Information);
	//printf("%s \n", Information);
	...
	if(RecordData->ProcessId != 4)
	{
		...
		//printf("make post data \n");
		char* post_data;
		post_data = (char*)malloc((strlen("Opr=") + strlen(Opr) + strlen("SeqNum=") + strlen(SeqNum) + strlen("Time=") + strlen(TIME) + strlen("PId=") + strlen(PId) + strlen("ProcessName=") + strlen(ProcessName) + strlen("MajorFunction=") + strlen(MajorFunction) + strlen("MinorFunction=") + strlen(MinorFunction) + strlen("Status=") + strlen(Status) + strlen("Information=") + strlen(Information) + strlen("Path=") + strlen(Path) + 1) * sizeof(char*));
		strcpy(post_data, "Opr=");
		strcat(post_data, Opr);
		strcat(post_data, "&SeqNum=");
		strcat(post_data, SeqNum);
		strcat(post_data, "&PId=");
		strcat(post_data, PId);
		strcat(post_data, "&ProcessName=");
		strcat(post_data, ProcessName);
		strcat(post_data, "&Time=");
		strcat(post_data, TIME);
		strcat(post_data, "&MajorFunction=");
		strcat(post_data, MajorFunction);
		strcat(post_data, "&MinorFunction=");
		strcat(post_data, MinorFunction);
		strcat(post_data, "&Status=");
		strcat(post_data, Status);
		strcat(post_data, "&Information=");
		strcat(post_data, Information);
		...
		curl = curl_easy_init();
		if (curl) {
			curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1/ransomware_soc/insert_Log.php");
